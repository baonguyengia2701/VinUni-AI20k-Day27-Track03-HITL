...
async def node_commit(state):
    ...
    if state.get("escalation_answers") or state.get("human_choice") == "approve":
        ...
        action = "rejected"
    # Emit an AuditEntry summarising what was committed (or rejected)
    await audit(state, AuditEntry(
        agent_id=AGENT_ID,
        action="commit",
...
async def node_auto_approve(state):
    ...
    a = state["analysis"]
    action = _post(state)
    # Audit — no human was involved, decision is "auto"
    await audit(state, AuditEntry(
        agent_id=AGENT_ID,
        action="auto_approve",
        confidence=a.confidence,
...
async def node_escalate(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="escalate",
        reason=f"Asking {len(questions)} question(s) to reviewer",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
    ...
    # Audit AFTER resume — we now have the reviewer's answers
    await audit(state, AuditEntry(
        agent_id=AGENT_ID,
        action="escalate",
        confidence=a.confidence,
        ...
        reviewer_id=os.environ.get("GITHUB_USER"),
        decision="escalate",
        reason=f"Received {len(answers)} answer(s) from reviewer",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_synthesize(state):
    ...
    # Audit with the NEW (refined) confidence
    await audit(state, AuditEntry(
        agent_id=AGENT_ID,
        action="synthesize",
