    ...
    with console.status("[dim]LLM reviewing the diff...[/dim]"):
        analysis = llm.invoke([
            {"role": "system", "content": (
                ...
                "with 2-4 specific, context-rich questions that reference exact files and "
                "lines from the diff. These questions should help a human reviewer clarify "
                "intent, risk, and correctness."
            )},
...
def node_synthesize(state: ReviewState) -> dict:
    ...
    with console.status("[dim]LLM refining review with reviewer answers...[/dim]"):
        refined = llm.invoke([
            {"role": "system", "content": "Refine your code review using the reviewer's answers to your questions. Produce updated structured output."},
            {"role": "user", "content": (
                f"Original diff:\n{state['pr_diff']}\n\n"
